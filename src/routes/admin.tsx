...
const getStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const businesses = queryDb<{ count: number }>("SELECT COUNT(*) as count FROM businesses");
  const workers = queryDb<{ count: number }>("SELECT COUNT(*) as count FROM workers");
  const shifts = queryDb<{ count: number }>("SELECT COUNT(*) as count FROM shifts");
  const bookings = queryDb<{ count: number }>("SELECT COUNT(*) as count FROM bookings");
  const messages = queryDb<{ count: number }>("SELECT COUNT(*) as count FROM messages");
  const recentShifts = queryDb("SELECT id, role_type, date, status, created_at FROM shifts ORDER BY created_at DESC LIMIT 10");
  const recentWorkers = queryDb("SELECT id, first_name, last_name, role_type, is_verified FROM workers ORDER BY created_at DESC LIMIT 10");
  const recentBusinesses = queryDb("SELECT id, name, venue_type, membership_tier, membership_status FROM businesses ORDER BY created_at DESC LIMIT 10");
...
const verifyWorker = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    execDb(`UPDATE workers SET is_verified=1 WHERE id='${esc(data.workerId)}'`);
    return { success: true };
  });
