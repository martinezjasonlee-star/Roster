          ...
          {(["overview", "workers", "businesses"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                activeTab === tab ? "bg-[#0F172A] text-white" : "bg-white border border-slate-200 text-slate-600"
              }`}>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b text-left text-slate-400 text-xs">
                ...
                <tbody>
                  {(data.recentShifts || []).map((s: any) => (
                    <tr key={s.id} className="border-b border-slate-50">
                      <td className="py-2 pr-4 text-xs text-slate-400">{s.id.slice(0, 8)}</td>
                      ...
                      <td className="py-2 pr-4"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        s.status === "open" ? "bg-green-100 text-green-700" : s.status === "filled" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"
                      }`}>{s.status}</span></td>
                      <td className="py-2 text-xs text-slate-400">{new Date(s.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === "workers" && (
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            {data.recentWorkers?.length === 0 ? (
              <p className="text-slate-400 text-sm">No workers registered yet.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b text-left text-slate-400 text-xs">
                  ...
                  <tbody>
                    {(data.recentWorkers || []).map((w: any) => (
                      <tr key={w.id} className="border-b border-slate-50">
                        ...
                        <td className="py-2 pr-4 capitalize text-slate-500">{w.role_type}</td>
                        <td className="py-2 pr-4">{w.is_verified ? <span className="text-green-600 font-medium">✅</span> : <span className="text-slate-300">—</span>}</td>
                        <td className="py-2">
                          {!w.is_verified && (
                  ...
                  <tbody>
                    {(data.recentBusinesses || []).map((b: any) => (
                      <tr key={b.id} className="border-b border-slate-50">
                        ...
                        <td className="py-2 pr-4 capitalize text-slate-500">{b.venue_type}</td>
                        <td className="py-2 pr-4 capitalize">{b.membership_tier}</td>
                        <td className="py-2">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                            b.membership_status === "active" ? "bg-green-100 text-green-700" :
                            b.membership_status === "trial" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"
