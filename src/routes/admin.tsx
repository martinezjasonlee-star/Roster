...
const getStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const businesses = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT COUNT(*) as count FROM businesses"`).toString());
  const workers = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT COUNT(*) as count FROM workers"`).toString());
  const shifts = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT COUNT(*) as count FROM shifts"`).toString());
  const bookings = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT COUNT(*) as count FROM bookings"`).toString());
  const messages = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT COUNT(*) as count FROM messages"`).toString());
  const recentShifts = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT id, role_type, date, status, created_at FROM shifts ORDER BY created_at DESC LIMIT 10"`).toString());
  const recentWorkers = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT id, first_name, last_name, role_type, is_verified FROM workers ORDER BY created_at DESC LIMIT 10"`).toString());
  const recentBusinesses = JSON.parse(execSync(`sqlite3 -json /home/team/.data/agent-team-cc229006.db "SELECT id, name, venue_type, membership_tier, membership_status FROM businesses ORDER BY created_at DESC LIMIT 10"`).toString());
...
const verifyWorker = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { execSync } = await import("node:child_process");
    execSync(`sqlite3 /home/team/.data/agent-team-cc229006.db "UPDATE workers SET is_verified=1 WHERE id='${data.workerId}'"`);
    return { success: true };
  });
