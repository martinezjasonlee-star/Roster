...
const getMessages = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const result = execSync(`team-db "SELECT m.id, m.sender_type, m.sender_id, m.content, m.is_read, m.created_at FROM messages m WHERE m.sender_id='${data.userId}' OR m.sender_id IN (SELECT id FROM workers WHERE id='${data.userId}') ORDER BY m.created_at DESC LIMIT 50"`);
...
const sendMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    execSync(`team-db "INSERT INTO messages (id, sender_type, sender_id, booking_id, content) VALUES ('${id}', '${data.sender_type}', '${data.sender_id}', '${data.booking_id}', '${data.content.replace(/'/g, "''")}')"`);
