...
const submitRating = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    execSync(`team-db "INSERT INTO ratings (id, booking_id, rater_type, rater_id, subject_type, subject_id, score, review) VALUES ('${id}', '${data.booking_id}', '${data.rater_type}', '${data.rater_id}', '${data.subject_type}', '${data.subject_id}', ${data.score}, '${data.review.replace(/'/g, "''")}')"`);
    return { success: true };
  });
